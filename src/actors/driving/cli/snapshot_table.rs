...
use crate::app::ports::types::Snapshot;
...
impl Display for SnapshotTable {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        for s in &self.snapshots {
            ...
            widths[4] = widths[4].max(s.dataset_size.to_string().len());
        }
        ...
        for s in &self.snapshots {
            ...
            write!(f, "{}|", pad(&s.dataset_size.to_string(), widths[4]))?;
            writeln!(f)?;
        }
        writeln!(f, "{}", sep('-'))?;
        Ok(())
